// tests/e2e/attack-persist-before-verify-drive-create.mjs
//
// Repro CONTRARIANT (Lot 5, axe 4 — « persist-avant-vérif », APPLIQUÉ au
// chemin « Créer une organisation » sur Drive, décision coordinateur : même
// défaut IDENTIQUE sur ce chemin LIVE testé sur mobile). Symétrique de
// tests/e2e/attack-persist-before-verify-drive.mjs (chemin PROMOTION,
// `promoteDriveOrg`/`activateShareSpaceDrive`), pour le chemin CRÉATION
// (`createDriveOrg`/`activateCreateOrgDrive`) :
// `piloteo-drive-bridge.mjs#createDriveOrg` persistait `piloteo_storage_mode`
// = "org-drive" + `piloteo_drive_root_folder_id` DÈS que la genèse (manifeste
// + fiche owner) était écrite sur Drive — AVANT que
// `local-backend.js#activateCreateOrgDrive` n'ait republié les événements
...
//
// CORRECTIF (identique à celui appliqué au chemin promotion) :
// `createDriveOrg` n'active plus rien lui-même (il écrit seulement la genèse
// sur Drive et renvoie `rootFolderId`) ; `activateCreateOrgDrive` persiste
...
// pas un stub) :
//   1. AXE 4 (CASSÉ->TENU) : lancer « Créer une organisation » sur Drive,
...
//      sont INTACTES, `app.js` démarre normalement (pas de "Compte non
//      rattaché").
//   2. Contraste (nominal) : une création COMPLÈTE (Continuer cliqué,
//      migration vérifiée) bascule bien en "org-drive", données présentes,
//      owner = moi — la correction ne casse pas le chemin heureux
//      (createDriveOrg nominal + rollback restent conformes au contrat).
//
// OAuth (Google Identity Services) et le transport réseau (`fetch` vers
// googleapis.com) sont STUBBÉS (même patron que
// tests/e2e/org-onboarding-drive.mjs / tests/e2e/attack-persist-before-verify-drive.mjs).
//
// Usage : node tests/e2e/attack-persist-before-verify-drive-create.mjs
...
// Services que les autres e2e Drive de ce lot — injecté AVANT tout script de
// page (addInitScript, donc rejoué à CHAQUE rechargement, y compris le
// rechargement du repro lui-même).
    ...
    accounts: {
      oauth2: {
        initTokenClient(opts) {
          return {
            requestAccessToken() {
              setTimeout(() => opts.callback({ access_token: "fake-e2e-attack-create-token", expires_in: 3600 }), 0);
            },
          };
        },
      },
...
try {
  ...
  // (createDriveOrg mine un workspace FRAIS — sans rapport avec ce
  // workspaceId solo — mais ce dernier ne doit de toute façon jamais changer).
  // ==========================================================================
  ...
  const seedResult = await page.evaluate(async () => {
    ...
    // ce fichier RECHARGE réellement la page, donc app.js rend vraiment le
    // tableau de bord au boot.
    solo.consultants = [{ id: "c-axe4-create", nom: "Axe4 Create Repro", trigramme: "AXC", statut: "en poste",
    ...
    return { consultantPresent: (put.state.consultants || []).some((c) => c.id === "c-axe4-create") };
  });
  ok(seedResult.consultantPresent, "donnée solo (c-axe4-create) seedée AVANT le repro");

  // ==========================================================================
  // 1. AXE 4 (CASSÉ->TENU) : lance `_createOrgDrive` (VRAIE
  //    `activateCreateOrgDrive`), SANS attendre sa résolution — la genèse
  ...
  await page.evaluate(() => {
    ...
    window.__axe4CreatePromise = window.PiloteoLocal._createOrgDrive("Cabinet Axe4 Create Repro").catch(() => {});
  });
  ...
  const afterReload = await page.evaluate(async () => {
    ...
    return {
      ...
      storedMode: (() => { try { return localStorage.getItem("piloteo_storage_mode"); } catch (e) { return null; } })(),
      storedRootFolderId: (() => { try { return localStorage.getItem("piloteo_drive_root_folder_id"); } catch (e) { return null; } })(),
      consultantPresent: (st.state?.consultants || []).some((c) => c.id === "c-axe4-create"),
  ...
  ok(afterReload.storageMode !== "org-drive", `AXE 4 TENU (création) : au reboot après reload, le mode N'EST PAS "org-drive" (reçu ${afterReload.storageMode})`);
  ok(!afterReload.orgDriveStorageMode, "AXE 4 TENU (création) : _orgDriveStorageMode() est faux après le reboot");
  ok(afterReload.storedMode !== "org-drive", `AXE 4 TENU (création) : piloteo_storage_mode reste PAS "org-drive" en localStorage après le reboot (reçu ${afterReload.storedMode})`);
  ok(!afterReload.storedRootFolderId, `AXE 4 TENU (création) : piloteo_drive_root_folder_id reste ABSENT après le reboot (reçu ${afterReload.storedRootFolderId})`);
  ok(afterReload.consultantPresent, "AXE 4 TENU (création) : la donnée solo (c-axe4-create) est INTACTE après le reboot (jamais perdue dans un org-drive vide inaccessible)");
  ok(afterReload.consultantCount === 1, `AXE 4 TENU (création) : aucune donnée fantôme/dupliquée après le reboot (reçu ${afterReload.consultantCount} consultant(s))`);
  ok(afterReload.meOk, "AXE 4 TENU (création) : /api/me répond normalement après le reboot (jamais le crash « Compte non rattaché à un consultant » d'un org-drive vide)");
  ok(afterReload.soloWorkspaceIdAfter === soloWorkspaceIdBefore, "AXE 4 TENU (création) : workspaceId solo INCHANGÉ après le reboot");
  ...
  // "Failed to load resource…") — même filtre que
  // tests/e2e/attack-persist-before-verify-drive.mjs/auth-session.mjs.
  const unexpectedAxe4Errors = consoleErrors.filter((e) => e.indexOf("Failed to load resource") === -1);
  ok(unexpectedAxe4Errors.length === 0, `aucune erreur console inattendue pendant le repro axe 4 création (${unexpectedAxe4Errors.length}/${consoleErrors.length})`);
  ...
  //    casse pas le chemin heureux de `createDriveOrg`/`activateCreateOrgDrive`.
  //    (Dismiss l'écran d'accueil réapparu au reboot solo — comportement
  //    normal du mode solo, sans rapport avec le repro.)
  // ==========================================================================
  await page.click("#piloteo-welcome button").catch(() => {});
  consoleErrors.length = 0;
  await page.evaluate(() => {
    window.__driveForFetch = new window.__FakeDriveE2E();
    window.__nominalCreatePromise = window.PiloteoLocal
      ._createOrgDrive("Cabinet Axe4 Create Nominal")
  ...
  const nominalOutcome = await page.evaluate(async () => {
    const o = await window.__nominalCreatePromise;
  ...
  ok(nominalOutcome.ok, `création nominale complète résout avec succès (erreur éventuelle: ${nominalOutcome.error})`);
  if (nominalOutcome.ok) {
    ...
    ok(nominalOutcome.storedRootFolderId === nominalOutcome.rootFolderId, "piloteo_drive_root_folder_id persisté (== rootFolderId de la création)");
    ok(nominalOutcome.meRole === "admin", `/api/me renvoie role=admin (reçu ${nominalOutcome.meRole})`);
    ok(nominalOutcome.ownerRole === "owner", `owner == créateur (reçu ${nominalOutcome.ownerRole})`);
  }

  // ==========================================================================
  // 3. ROLLBACK (non-régression) : échec de la vérification finale (Point 5,
  //    `__forceNextVerificationFailure`) -> jamais de bascule org-drive,
  //    `piloteo_storage_mode`/`piloteo_drive_root_folder_id` NETTOYÉS (sécurité
  //    secondaire toujours fonctionnelle), solo intact.
  // ==========================================================================
  ...
  const rollbackSetup = await page.evaluate(async () => {
    ...
    solo.consultants = [{ id: "c-axe4-create-rollback", nom: "Rollback Create", trigramme: "RBC", statut: "en poste",
      dateEmbauche: "2026-01-01", dateDepart: null, tjmBase: 500, admin: false, tempsPartiel: [] }];
    const put = await (await fetch("/api/state", { method: "PUT", body: JSON.stringify({ state: solo }) })).json();
    return { consultantPresent: (put.state.consultants || []).some((c) => c.id === "c-axe4-create-rollback") };
  });
  ok(rollbackSetup.consultantPresent, "donnée solo (distincte) seedée AVANT le scénario de rollback (création)");

  consoleErrors.length = 0;
  await page.evaluate(() => {
    window.__driveForFetch = new window.__FakeDriveE2E();
    window.PiloteoMigration.__forceNextVerificationFailure();
    window.__rollbackCreatePromise = window.PiloteoLocal
      ._createOrgDrive("Cabinet Axe4 Rollback Création")
      .then((r) => ({ ok: true, value: r }))
      .catch((e) => ({ ok: false, error: e && e.message }));
  });
  ...
  const outcomeRollback = await page.evaluate(async () => {
    const o = await window.__rollbackCreatePromise;
    const st = await (await fetch("/api/state")).json();
    return {
      ok: o.ok,
      error: o.error,
      ...
      storedMode: (() => { try { return localStorage.getItem("piloteo_storage_mode"); } catch (e) { return null; } })(),
      storedRootFolderId: (() => { try { return localStorage.getItem("piloteo_drive_root_folder_id"); } catch (e) { return null; } })(),
      consultantStillPresent: (st.state?.consultants || []).some((c) => c.id === "c-axe4-create-rollback"),
    };
  });

  ok(!outcomeRollback.ok, "_createOrgDrive() est REJETÉE après un échec de vérification (ROLLBACK, non-régression)");
  ...
  ok(!outcomeRollback.storedRootFolderId, "piloteo_drive_root_folder_id NETTOYÉ après le rollback");
  ok(outcomeRollback.consultantStillPresent, "les données solo (consultant c-axe4-create-rollback) restent INTACTES après le rollback");
